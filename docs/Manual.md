...
We have a schema file can be used to validate the EaseProbe configuration file. It is available at [resources/schema.json](https://raw.githubusercontent.com/megaease/easeprobe/main/resources/schema.json).

This schema file can be generated by the following command:

```bash
$ easeprobe -j > schema.json
```

For VSCode, you can install the [YAML extension](https://marketplace.visualstudio.com/items?itemName=redhat.vscode-yaml) and add the following configuration to your `settings.json` file:
