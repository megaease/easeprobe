...
## Features
The project principles, of EaseProbe' features for are separated into two main categories: probe-specific & notify-specific.
...
## Roadmap 2022
Some of the features that are planned/considered for 2022 can be found broken down into three categories: *General*, *Probe*, *Notify*


### General
* Work on detailed documentation
* support for common daemon features
  * [x] add `daemon()` & `/var/run/easeprobe.pid` support
  * [x] add SIGHUP, and ensure it closes and re-opens of logfile to allow for `easeprobe.log` rotation
  * add syslog support `logfile: syslog(facility.level)`
* `timeformat` why not use standard timezone and `strftime` conversions? eg `timezone: [UTC|local|Europe/Athens]`, `timeformat: %F %R:%S UTC`
...
* Add support to define desired notification on probes??? (_This maybe need a bit of discussion_)
```yaml
tcp:
  - name: Memcached
...
```
* Add support for host group probes (eg 1 host definition with 4 services) (_I would like to discuss these as I am not strongly for them but still came across the need for them when i was working with easeprobe_)
```yaml
name: MyServer
  probes:
    tcp:
...
### Notify
* export the status data to open formats so it can easily be integrated with 3rd party applications such as Prometheus and Graphana
...
* work on clear distinction between `host`, `ssh` and `shell` (certain areas seem overlapping):
  ...
  * check that we are OS agnostic where possible and confirm OS specific operations are abstracted (`/proc/cpuinfo` not on all OS's)
